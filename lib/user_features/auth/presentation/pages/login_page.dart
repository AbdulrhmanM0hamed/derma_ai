...
class _LoginPageState extends State<LoginPage> {
  ...
  void _login() async {
    if (_formKey.currentState!.validate()) {
      ...
      CustomSnackbar.showError(
        ...
        message: CustomSnackbar.getLocalizedMessage(
          context: context,
          messageAr: 'حدث خطأ أثناء حفظ بيانات الذاكرة',
          messageEn: 'An error occurred while saving remember me data',
        ),
      );
     }
  ...
  void _saveRememberedEmail() async {
    try {
      await sl<TokenStorageService>().setRememberMe(
        ...
        password: _rememberMe ? _passwordController.text.trim() : null,
      );
    } catch (e) {
      CustomSnackbar.showError(
        context: context,
  ...
  void _clearRememberedEmail() async {
    try {
      await sl<TokenStorageService>().clearRememberMe();
    } catch (e) {
      CustomSnackbar.showError(
        context: context,
        message: CustomSnackbar.getLocalizedMessage(
          context: context,
          messageAr: 'حدث خطأ أثناء حذف بيانات الذاكرة',
          messageEn: 'An error occurred while clearing remember me data',
        ),
      );
    }
  }
  ...
  Widget _buildSignUpLink() {
    ...
    ).animate(effects: fadeInSlide(duration: 600.ms, delay: 800.ms, beginY: 0.2));
  }
}
